...
| 数字分隔符 🔒 | `_` 仅允许出现于两个数字之间；`1000_`、`1__000` 编译错；`_1000` 为标识符（首字符分流，语义见 [15-数据类型](./15-数据类型-TYPES.md) §15.2） |
| 转义集封闭 🔒 | 字符串内反斜杠只认 escape 产生式所列五项，未知转义编译错（[15-数据类型](./15-数据类型-TYPES.md) §15.3） |
